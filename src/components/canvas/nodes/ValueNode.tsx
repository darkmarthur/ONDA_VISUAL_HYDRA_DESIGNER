...
import { Activity, MousePointer2, Clock, Hash, Percent, Divide } from 'lucide-react';
...
function ValueNode({ id, data, selected }: NodeProps & { data: HydraNodeData }) {
  ...
  return (
    <div
      className={`hydra-node hydra-node--value ${selected ? 'hydra-node--selected' : ''}`}
               ...
               return (
                 <div key={p.name} className={`hydra-node__param-preview ${isBound ? 'hydra-node__param-preview--bound' : ''}`}>
                   ...
                   <span className="hydra-node__param-name">{p.name}</span>
                    <input
                      className={`hydra-node__param-input ${isBound ? 'hydra-node__param-input--binding' : ''}`}
                      type={isBound ? "text" : "number"}
                      step={p.step || 0.01}
                      value={isBound ? (binding.expression || '') : (data.params[p.name] ?? p.default)}
                      onClick={(e) => e.stopPropagation()}
                      onChange={(e) => {
                        if (isBound) {
                          updateNodeBinding(id, p.name, { expression: e.target.value });
                        } else {
